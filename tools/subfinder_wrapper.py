...
class SubfinderTool(BaseTool):
    ...
    def _resolve_executable(self) -> Path:
        ...
        if configured_path:
            ...
            if candidate.exists():
                return candidate
            logger.debug(f"Subfinder 配置路径不存在，回退到默认路径: {candidate}")
    ...
    def install(self) -> bool:
        ...
        提示用户手动安装
        """
        logger.warning(f"[yellow]请手动下载 Subfinder 并放置到: {self.tool_dir}[/yellow]")
    ...
    def scan(self, domain: str) -> List[str]:
        ...
        if not self.is_installed():
            logger.error(f"Subfinder 未安装，请将可执行文件放到: {self.tool_dir}")
            return []
        ...
        try:
            ...
            logger.info(f"[green]Subfinder 发现 {len(subdomains)} 个子域名[/green]")
            return sorted(list(subdomains))
            
        except subprocess.TimeoutExpired:
            logger.error("Subfinder 扫描超时")
            return []
        except Exception as e:
            logger.error(f"Subfinder 扫描出错: {e}")
            return []
